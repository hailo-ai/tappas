...
pushd $TAPPAS_WORKSPACE/hailort/sources/
rm -rf build

cmake -H. -Bbuild -DHAILO_BUILD_GSTREAMER=1 -DCMAKE_BUILD_TYPE=$BUILD_TYPE
cmake --build build

sudo cp -a $TAPPAS_WORKSPACE/hailort/sources/build/hailort/libhailort/bindings/gstreamer/libgsthailo.so /usr/lib/$(uname -p)-linux-gnu/gstreamer-1.0
rm -rf $TAPPAS_WORKSPACE/hailort/sources/build/hailort/libhailort/src/libhailort.so*
popd
