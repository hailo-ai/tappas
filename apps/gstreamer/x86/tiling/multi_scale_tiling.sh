...
function init_variables() {
    ...
    readonly RESOURCES_DIR="$TAPPAS_WORKSPACE/apps/gstreamer/x86/tiling/resources"

    readonly DEFAULT_DETECTION_POSTPROCESS_SO="$POSTPROCESS_DIR/libmobilenet_ssd_post.so"
    readonly DEFAULT_DETECTION_VIDEO_SOURCE="$RESOURCES_DIR/street_night.mp4"
    ...
    postprocess_func_name="mobilenet_ssd"
...
function parse_args() {
    while test $# -gt 0; do
        if [ "$1" = "--print-gst-launch" ]; then
            ...
            iou_threshold="$2"
            shift
        elif [ "$1" = "--border-threshold" ]; then
            border_threshold="$2"
            shift
        elif [ "$1" = "--scale-level" ]; then
            scale_level="$2"
            shift
...
PIPELINE="gst-launch-1.0 $source_element ! \
    ...
    $TILE_CROPPER_ELEMENT hailotileaggregator iou-threshold=$iou_threshold border-threshold=$border_threshold name=agg \
