...
function init_variables() {
    ...
    readonly DEFAULT_VIDEO_SOURCE="$TAPPAS_WORKSPACE/apps/h8/gstreamer/raspberrypi/multinetworks_parallel/resources/instance_segmentation.mp4"
    readonly DEFAULT_HEF_PATH="$TAPPAS_WORKSPACE/apps/h8/gstreamer/raspberrypi/multinetworks_parallel/resources/joined_fast_depth_ssd_mobilenet_v1_no_alls.hef"
...
PIPELINE="gst-launch-1.0 \
    ...
    vdevice-key=$DEFAULT_VDEVICE_KEY net-name=$depth_estimation_net_name output-format-type=HAILO_FORMAT_TYPE_FLOAT32 ! \
    queue leaky=no max-size-buffers=10 max-size-bytes=0 max-size-time=0 ! \
    ...
    queue leaky=no max-size-buffers=10 max-size-bytes=0 max-size-time=0 ! \
    hailonet hef-path=$hef_path is-active=true scheduling-algorithm=0 \
    vdevice-key=$DEFAULT_VDEVICE_KEY net-name=$detection_net_name output-format-type=HAILO_FORMAT_TYPE_FLOAT32 ! \
    queue leaky=no max-size-buffers=10 max-size-bytes=0 max-size-time=0 ! \
    ...
    queue leaky=no max-size-buffers=10 max-size-bytes=0 max-size-time=0 ! \
    videoconvert ! \
    fpsdisplaysink video-sink=ximagesink name=hailo_display2 sync=false text-overlay=false ${additional_parameters} "
