...
function print_usage() {
    ...
    echo "  --use-overlay                   Whether the pipeline should include the overlay (overlay could slow down the pipeline)."
...
function create_sources() {
    ...
    for ((n = $start_index; n < $num_of_src; n++)); do
        sources+="filesrc location=$RESOURCES_DIR/detection$n.mp4 name=source_$n ! \
                  parsebin ! \
                  queue name=hailo_before_mpp_$n leaky=no max-size-buffers=5 max-size-bytes=0 max-size-time=0 ! \
                  ...
                  queue name=comp_export_$n leaky=no max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! \
                  hailoexportfile location='$TAPPAS_WORKSPACE/metadata_$n.json' name=export_$n ! \
                  queue name=comp_enc_$n leaky=no max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! \
...
function main() {
    ...
    pipeline="gst-launch-1.0 \
             ...
             hailofilter function-name=yolov5s_nv12 so-path=$POSTPROCESS_SO qos=false ! \
             hmux. hmux. ! \
             $overlay_element \
