...
function create_sources() {
    for ((n = 0; n < $num_of_src; n++)); do
        sources+="uridecodebin3 uri=file://$RESOURCES_DIR/detection$n.mp4 name=source_$n ! \
                ...
                hailonet hef-path=$HEF_PATH scheduling-algorithm=1 batch-size=1 vdevice-key=1 nms-score-threshold=0.3 nms-iou-threshold=0.45 output-format-type=HAILO_FORMAT_TYPE_FLOAT32 ! \
