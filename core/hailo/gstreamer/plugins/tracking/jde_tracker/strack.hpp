...
class STrack
{
    ...
    NewHailoDetectionPtr m_hailo_detection; // A shared pointer to the detection object in the pipeline
    HailoClassificationPtr m_shadow_classification;
    HailoClassificationPtr m_hailo_classification; // A shared pointer to a classification to apply to the detection 
    ...
    STrack(std::vector<float> tlwh_ = {0., 0., 0., 0.}, float score_ = 0.0, std::vector<float> temp_feat = {0.0},
           NewHailoDetectionPtr detection_ptr = nullptr, HailoClassificationPtr classification_ptr = nullptr) : 
    m_is_activated(false), m_track_id(0), m_frame_id(0), m_tracklet_len(0), m_confidence(score_),
    m_start_frame(0), m_alpha(0.9), tmp_location_tlwh(tlwh_), m_state(TrackState::New),
    m_hailo_detection(detection_ptr), m_hailo_classification(classification_ptr)
    {
        m_times_seen = 0;
        m_shadow_classification = nullptr;
    ...
    public:
    /**
     * @brief Adds the HailoUniqueID to the held NewHailoDetectionPtr
     * 
     */
    void update_hailo_id()
    {
    ...
    /**
     * @brief Update the NewHailoDetectionPtr bbox with the strack's m_tlwh
     * 
     */
    ...
    /**
     * @brief Update the NewHailoClassificationPtr bbox with the strack's m_tlwh
     * 
     */
    void update_hailo_classification()
    {
        if (nullptr != this->m_hailo_classification)
        {
            hailo_common::add_classification(this->m_hailo_detection,
                                             this->m_hailo_classification->get_classification_type(),
                                             this->m_hailo_classification->get_label(),
                                             this->m_hailo_classification->get_confidence());
        }
    }

    void add_classification(HailoClassificationPtr new_classification)
    {
        this->m_hailo_classification = new_classification;
    }
    ...
    NewHailoDetectionPtr get_hailo_detection() { return this->m_hailo_detection; }

    // Get the hailo classification object
    HailoClassificationPtr get_hailo_classification() { return this->m_hailo_classification; }
    ...
    void re_activate(STrack &new_track, int frame_id, bool new_id)
    {
        ...
        // Update the NewHailoDetectionPtr to the new track and update it's id
        this->m_hailo_detection = new_track.get_hailo_detection();
        update_hailo_classification();
        update_hailo_id();
        update_hailo_bbox();
    }
