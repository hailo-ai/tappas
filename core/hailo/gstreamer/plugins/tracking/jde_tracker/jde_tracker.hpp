...
class JDETracker
{
    ...
    float m_conf_thresh;      // threshold used to filter new un-matched detections, smaller is looser
    ...
    public:
    // Default Constructor
    JDETracker(float kalman_dist = 0.7, float iou_thr = 0.8, float init_iou_thr = 0.9, float conf_thresh = 0.5,
               int keep_tracked = 2, int keep_new = 2, int keep_lost = 2,
               float std_weight_position = 0.01, float std_weight_pos_box = 0.01, float std_weight_velocity = 0.001, float std_weight_vel_box = 0.001) : 
    m_kalman_dist_thr(kalman_dist), m_iou_thr(iou_thr), m_init_iou_thr(init_iou_thr), m_conf_thresh(conf_thresh),
    ...
    /******************** PUBLIC FUNCTIONS ****************************/
    public:
    static std::vector<STrack> hailo_detections_to_stracks(std::vector<NewHailoDetectionPtr> &inputs);
    static std::vector<NewHailoDetectionPtr> stracks_to_hailo_detections(std::vector<STrack> &stracks);
    STrack* get_detection_with_id(int track_id);
    std::vector<STrack> get_tracked_stracks();
    std::vector<HailoClassificationPtr> get_tracked_classifications();
    std::vector<STrack> update(std::vector<NewHailoDetectionPtr> &inputs, bool report_unconfirmed);
