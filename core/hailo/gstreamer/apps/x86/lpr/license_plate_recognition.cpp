...
const std::string DEFAULT_VIDEO_SRC = RESOURCES_DIR + "lpr_ayalon.mp4";
...
std::string create_pipeline_string(std::string input_src)
{
    ...
    if (input_src.rfind("/dev/video", 0) == 0)
    {
        ...
        src_pipeline_string = "filesrc name="+std::string(PIPELINE_SRC)+" location=" + input_src + " name=src_0 ! decodebin qos=false";
    ...
    vehicle_detection_subpipeline = src_pipeline_string + " ! "
                                    ...
                                    CONTEXT_0_T_SRC + " ! "
                                        ...
                                        "videoconvert ! "
                                        "fpsdisplaysink video-sink=xvimagesink name="+CONTEXT_0_DISPLAY+" sync=true async-handling=true text-overlay=false signal-fps-measurements=true";
    ...
    license_plate_detection_subpipeline =  " " + CONTEXT_0_T_SRC + " ! "
                                           ...
                                           "appsink sync=false async=true qos=false wait-on-eos=false emit-signals=true name=" + CONTEXT_1_APP_SINK;
...
static GstFlowReturn context_1_ocr_results_callback(GstAppSink * sink, gpointer udata)
{
    ...
    buffer = gst_buffer_copy(buffer);           // Get a read/writeable copy
