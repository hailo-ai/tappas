...
// Basic Directories
std::string POSTPROCESS_DIR;
std::string CROPPING_ALGORITHMS_DIR;
std::string RESOURCES_DIR;
const std::string DEFAULT_VIDEO_NAME = "lpr.raw";
std::string VIDEO_SRC;

// Vehicle Detection Macros
std::string VEHICLE_DETECTION_HEF;
std::string VEHICLE_DETECTION_POST;
const std::string VEHICLE_DETECTION_POST_FUNC = "yolov5_vehicles_only";

// License Plate Detection and OCR Macros
// HEF and net names
std::string LICENSE_PLATE_HEF;
...
static void set_app_globals(int argc, char *argv[])
{
    // Basic Directories
    POSTPROCESS_DIR = "/usr/lib/hailo-post-processes/";
    RESOURCES_DIR = "/home/root/apps/license_plate_recognition/resources/";
    ...
    // Basic Properties
    VIDEO_SRC = RESOURCES_DIR + DEFAULT_VIDEO_NAME;

    // Vehicle Detection Macros
    VEHICLE_DETECTION_HEF = RESOURCES_DIR + "yolov5m_vehicles_no_ddr_yuy2.hef";
    VEHICLE_DETECTION_POST = POSTPROCESS_DIR + "libnew_yolo_post.so";

    // HEF and net names
    LICENSE_PLATE_HEF = RESOURCES_DIR + "joined_tiny_yolov4_license_plates_lprnet_yuy2.hef";

    // Post-process .so's and function names
    LICENSE_PLATE_DETECTION_POST = POSTPROCESS_DIR + "libnew_yolo_post.so";
    LICENSE_PLATE_OCR_POST = POSTPROCESS_DIR + "libocr_post.so";

    // Cropping algorithm .so and function names
    LICENSE_PLATE_CROP_SO = CROPPING_ALGORITHMS_DIR + "liblpr_croppers.so";
}
...
std::string create_pipeline_string(std::string input_src)
{
    ...
    if (input_src.rfind("/dev/video", 0) == 0)
    {
        ...
        src_pipeline_string = "filesrc name="+std::string(PIPELINE_SRC)+" location=" + input_src + " name=src_0 ! rawvideoparse format=yuy2 width=1920 height=1080";
    ...
    vehicle_detection_subpipeline = src_pipeline_string + " ! "
                                    ...
                                    "hailonet name="+CONTEXT_0_DETECTION_NET+" hef-path="+VEHICLE_DETECTION_HEF+" device-count=1 vdevice-key=1 is-active=true qos=false ! "
                                    "queue name=q1 leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! "
                                    "hailofilter2 so-path=" + VEHICLE_DETECTION_POST + " function-name=" + VEHICLE_DETECTION_POST_FUNC + "  qos=false ! "
                                    ...
                                    "tee name="+CONTEXT_0_T+" "+CONTEXT_0_T_SRC+" ! "
                                        "identity drop-allocation=true ! "
                                        "queue name=q4 leaky=no max-size-buffers=100 min-threshold-buffers=0 max-size-bytes=0 max-size-time=0 ! "
                                        "hailooverlay line-thickness=2 font-thickness=1 qos=false ! "
                                        "fpsdisplaysink video-sink=autovideosink name="+CONTEXT_0_DISPLAY+" sync=true text-overlay=false signal-fps-measurements=true";
    ...
    license_plate_detection_subpipeline =  " " + CONTEXT_0_T_SRC + " ! "
                                           ...
                                           "hailocropper drop-uncropped-buffers=true internal-offset="+internal_offset+" so-path=" + LICENSE_PLATE_CROP_SO + " function-name=" + LICENSE_PLATE_DETECTION_CROP_FUNC + " name=cropper1 "
                                           ...
                                           "cropper1. ! "
                                                ...
                                                "hailonet name="+CONTEXT_1_DETECTION_NET+" hef-path=" + LICENSE_PLATE_HEF + " net-name=" + LICENSE_PLATE_DETECTION_NET_NAME + " device-count=1 vdevice-key=1 is-active=false qos=false ! "
                                                "queue name=lp_detection_q1 leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! "
                                                "hailofilter2 so-path=" + LICENSE_PLATE_DETECTION_POST + " function-name=" + LICENSE_PLATE_DETECTION_POST_FUNC + "  qos=false ! "
                                           ...
                                           "hailocropper drop-uncropped-buffers=true method=bilinear internal-offset="+internal_offset+" so-path=" + LICENSE_PLATE_CROP_SO + " function-name=" + LICENSE_PLATE_OCR_CROP_FUNC + "  name=cropper2 "
                                           ...
                                           "cropper2. ! "
                                                ...
                                                "hailonet name="+CONTEXT_1_OCR_NET+" hef-path=" + LICENSE_PLATE_HEF + " net-name=" + LICENSE_PLATE_OCR_NET_NAME + " device-count=1 vdevice-key=1 is-active=false qos=false ! "
                                                "queue name=lp_ocr_q1 leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! "
                                                "hailofilter2 so-path=" + LICENSE_PLATE_OCR_POST + " qos=false ! "
                                                "queue name=lp_ocr_q2 leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! "
                                           "agg2. "
                                           "agg2. ! queue  name=q9 leaky=no max-size-buffers=100 max-size-bytes=0 max-size-time=0 ! "
...
/**
 * @brief Handle bus messages from the pipeline (EOS, errors, etc...)
 * 
 * @param bus  -  GstBus *
 *        The pipeline bus.
 * 
 ...
 * 
 * @param udata  -  gpointer
 *        User data, in this case the GMainLoop *
 *        of the application loop.
 * 
 * @return gboolean 
 *         Whether or not to continue handling bus messages
 */
static gboolean bus_message_handling(GstBus *bus, GstMessage *message, gpointer udata)
{
...
int main(int argc, char *argv[])
{
    ...
    // Create the pipeline
    std::string pipeline_string = create_pipeline_string(VIDEO_SRC);
    GstElement *pipeline = gst_parse_launch(pipeline_string.c_str(), NULL);
    bus = gst_element_get_bus(pipeline);
    bus_watch_id = gst_bus_add_watch(bus, bus_message_handling, loop);
