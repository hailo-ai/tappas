...
const std::string HAILONET_1_POST = POSTPROCESS_DIR + "libyolo_post.so";
const std::string HAILONET_1_DRAW = POSTPROCESS_DIR + "libdetection_draw.so";
...
std::string create_pipeline_string(std::string input_src)
{
    ...
    pipeline_branch_1 = "t. ! "
                        ...
                        std::string(HAILONET1_NAME) + " ! "
                                                      "queue leaky=no max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! "
                                                      "hailofilter function-name=" +
                        POST_1_FUNC_NAME +
                        " so-path=" +
                        tappas_workspace + HAILONET_1_POST + " qos=false debug=False ! "
                                                             "queue leaky=no max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! "
                                                             "hailofilter so-path=" +
                        tappas_workspace + HAILONET_1_DRAW + " qos=false debug=False ! "
    ...
    pipeline_branch_2 = "t. ! "
                        ...
                        std::string(HAILONET2_NAME) + " ! "
                                                      "queue leaky=no max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! "
                                                      "hailofilter so-path=" +
                        tappas_workspace + HAILONET_2_POST + " qos=false debug=False ! "
                                                             "queue leaky=no max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! "
                                                             "videoconvert qos=false ! queue name=hailo_display_q_1 leaky=no max-size-buffers=1 "
