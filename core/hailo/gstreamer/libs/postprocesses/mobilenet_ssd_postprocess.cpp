...
#include <vector>
#include <string>
#include "mobilenet_ssd_postprocess.hpp"
...
{
private:
    NewHailoTensorPtr _nms_output_tensor;
    ...
    void parse_bbox_to_detection_object(common::hailo_bbox_t *bbox_struct, uint32_t class_index, std::vector<NewHailoDetection>& _objects)
    {
        ...
        if (dequant_bbox.score > _detection_thr)
        {
            ...
            _objects.push_back(NewHailoDetection(HailoBBox(dequant_bbox.x_min, dequant_bbox.y_min, w, h), class_index, labels_dict[class_index], confidence));
        }
    }
...
public:
    MobilenetSSDPost(NewHailoTensorPtr tensor, std::map<uint8_t, std::string> &labels_dict, float detection_thr = DEFAULT_MOBILENET_THRESHOLD, uint max_boxes = DEFAULT_MAX_BOXES)
    ...
    std::vector<NewHailoDetection> decode()
    {
        ...
        std::vector<NewHailoDetection> _objects;
