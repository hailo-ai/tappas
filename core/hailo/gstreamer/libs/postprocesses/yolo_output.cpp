...
std::pair<uint, float> YoloOutputLayer::get_class(uint row, uint col, uint anchor)
{
    ...
    for (uint class_id = 1; class_id <= _num_classes; ++class_id)
    {
...
{
    uint channel = _tensor->channels / NUM_ANCHORS * anchor + CONF_CHANNEL_OFFSET;
    return _tensor->get_full_percision(row, col, channel);
}

uint Yolov5OL::get_class_prob(uint row, uint col, uint anchor, uint class_id)
{
    uint channel = _tensor->channels / NUM_ANCHORS * anchor + CLASS_CHANNEL_OFFSET + class_id - 1;
    return _tensor->get(row, col, channel);
}

float Yolov5OL::get_class_conf(uint prob_max)
{
    return _tensor->fix_scale(prob_max);
}

std::pair<float, float> Yolov5OL::get_center(uint row, uint col, uint anchor)
{
    float x, y = 0.0f;
    uint channel = _tensor->channels / NUM_ANCHORS * anchor;
...
{
    float w, h = 0.0f;
    uint channel = _tensor->channels / NUM_ANCHORS * anchor + NUM_CENTERS;
...
{
    float w, h = 0.0f;
    uint channel = (_scale->channels / NUM_ANCHORS) * anchor;
...
float YoloSplittedOutputLayer::sigmoid(float x)
{
...
{
    uint channel = (_center->channels / NUM_ANCHORS) * anchor;
