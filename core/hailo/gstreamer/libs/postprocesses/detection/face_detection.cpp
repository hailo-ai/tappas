...
void retinaface(HailoROIPtr roi)
{
    ...
    std::vector<HailoDetection> detections = face_detection_postprocess(tensors, anchors, anchors_multiplier, anchor_variance,
...
//******************************************************************
std::vector<HailoDetection> lightface_post(std::vector<HailoTensorPtr> &tensors)
{
    /*
...
{
    std::vector<HailoTensorPtr> tensors = roi->get_tensors();
    std::vector<HailoDetection> detections = lightface_post(tensors);
    hailo_common::add_detections(roi, detections);
}
