 ...
 */
std::pair<xt::xarray<int>, xt::xarray<uint8_t>> top_k_centers(NewHailoTensorPtr scores, const int k)
{
 ...
 * @return std::pair<xt::xarray<int>, xt::xarray<uint8_t>> pair of indices of scores and scores
 */
std::pair<xt::xarray<int>, xt::xarray<uint8_t>> top_k_joints(NewHailoTensorPtr joint_scores, const int k)
{
    // Adapt the tensor into an xarray of proper shape and size
 ...
 */
xt::xarray<uint8_t> gather_features_from_tensor(NewHailoTensorPtr tensor, xt::xarray<int> &indices)
{
    // Adapt the tensor into an xarray of proper shape and size:
 ...
 * @param image_size image width/height (the netowork input tensor is square)
 */
void encode_boxes_centerpose(std::vector<NewHailoDetection> &objects,
                             ...
                             const int image_size)
{
    ...
    for (int index = 0; index < max_detections; index++)
    {
        ...
        NewHailoDetection detected_pose(HailoBBox(xmin, ymin, w, h), -1, label, confidence);
 ...
 * @return std::vector<NewHailoDetection> the detected objects
 */
std::vector<NewHailoDetection> centerpose_postprocess(HailoROIPtr roi,
                                                      const int k,
                                                      const float score_threshold,
                                                      const float iou_thr)
{
    ...
    NewHailoTensorPtr center_heatmap = roi->get_tensor(CENTER_HEATMAP_OUTPUT_LAYER);
    NewHailoTensorPtr center_width_height = roi->get_tensor(CENTER_WIDTH_HEIGHT_OUTPUT_LAYER);
    NewHailoTensorPtr center_offset = roi->get_tensor(CENTER_OFFSET_OUTPUT_LAYER);
    // Joint heatmap and offset tensors for joint detection
    NewHailoTensorPtr joint_heatmap = roi->get_tensor(JOINT_HEATMAP_OUTPUT_LAYER);
    NewHailoTensorPtr joint_offset = roi->get_tensor(JOINT_OFFSET_OUTPUT_LAYER);
    // Joint center offset tensor for secondary joint detection
    NewHailoTensorPtr joint_center_offset = roi->get_tensor(JOINT_CENTER_OFFSET_OUTPUT_LAYER);
