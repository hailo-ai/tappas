...
std::vector<HailoROIPtr> license_plate_quality_estimation(cv::Mat image, HailoROIPtr roi)
{
    ...
    for (NewHailoDetectionPtr &detection : detections_ptrs)
    {
        ...
        std::vector<NewHailoDetectionPtr> inner_detection_ptrs = hailo_common::get_hailo_detections(detection);
        for(NewHailoDetectionPtr &inner_detection : inner_detection_ptrs)
        {
