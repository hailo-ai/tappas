...
void arcface(HailoROIPtr roi, gchar *current_stream_id, std::string layer_name)
{
    if (!roi->has_tensors())
    {
        return;
...
{
    arcface(roi, current_stream_id, OUTPUT_LAYER_NAME_RGB);
}

void arcface_rgba(HailoROIPtr roi, GstVideoFrame *frame, gchar *current_stream_id)
{
    arcface(roi, current_stream_id, OUTPUT_LAYER_NAME_RGBA);
}
