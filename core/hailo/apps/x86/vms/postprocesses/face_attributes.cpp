...
void face_attributes_postprocess(HailoROIPtr roi, GstVideoFrame *frame, gchar *current_stream_id, std::string output_layer_name)
{
    if (!roi->has_tensors())
    {
        return;
...
{
    face_attributes_postprocess(roi, frame, current_stream_id, RESNET_V1_18_FACE_OUTPUT_LAYER_NAME);
}

void face_attributes_rgba(HailoROIPtr roi, GstVideoFrame *frame, gchar *current_stream_id)
{
    face_attributes_postprocess(roi, frame, current_stream_id, "face_attr_resnet_v1_18_rgbx/fc3");
}
