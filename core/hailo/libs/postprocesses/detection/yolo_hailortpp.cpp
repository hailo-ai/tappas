...
static const std::string DEFAULT_YOLOV5M_OUTPUT_LAYER = "yolov5m_wo_spp/yolov5_nms_postprocess";
...
static const std::string DEFAULT_YOLOX_OUTPUT_LAYER = "yolox_nms_postprocess";
static const std::string DEFAULT_YOLOV5M_VEHICLES_NV12_OUTPUT_LAYER = "yolov5m_vehicles_nv12/yolov5_nms_postprocess";
static const std::string DEFAULT_YOLOV5S_PERSONFACE_OUTPUT_LAYER = "yolov5s_personface_nv12/yolov5_nms_postprocess";
...
{
    nms_decode_to_roi(roi, DEFAULT_YOLOV5M_OUTPUT_LAYER, common::coco_eighty);
}

void yolov5s_nv12(HailoROIPtr roi)
{
    nms_decode_to_roi(roi, DEFAULT_YOLOV5S_OUTPUT_LAYER, common::coco_eighty);
}

void yolov8s(HailoROIPtr roi)
{
    nms_decode_to_roi(roi, DEFAULT_YOLOV8S_OUTPUT_LAYER, common::coco_eighty);
}

void yolov8m(HailoROIPtr roi)
{
    nms_decode_to_roi(roi, DEFAULT_YOLOV8M_OUTPUT_LAYER, common::coco_eighty);
}

void yolox(HailoROIPtr roi)
{
    nms_decode_to_roi(roi, DEFAULT_YOLOX_OUTPUT_LAYER, common::coco_eighty);
}

void yolov5m_vehicles(HailoROIPtr roi)
{
    nms_decode_to_roi(roi, DEFAULT_YOLOV5M_VEHICLES_OUTPUT_LAYER, yolo_vehicles_labels);
}

void yolov5m_vehicles_nv12(HailoROIPtr roi)
{
    nms_decode_to_roi(roi, DEFAULT_YOLOV5M_VEHICLES_NV12_OUTPUT_LAYER, yolo_vehicles_labels);
}

void yolov5s_personface(HailoROIPtr roi)
{
    nms_decode_to_roi(roi, DEFAULT_YOLOV5S_PERSONFACE_OUTPUT_LAYER, common::yolo_personface);
}
...
void yolov5_no_persons(HailoROIPtr roi)
{
    ...
    auto detections = nms_decode(
        roi->get_tensor(DEFAULT_YOLOV5M_OUTPUT_LAYER), common::coco_eighty);
    detections.erase(
        std::remove_if(detections.begin(), detections.end(),
            [](const HailoDetection &det) { return "person" == det.get_label(); }),
        detections.end());
...
void filter(HailoROIPtr roi, void *params_void_ptr)
{
    ...
    const auto *params = reinterpret_cast<const YoloParamsNMS *>(params_void_ptr);
    for (const auto &tensor : roi->get_tensors()) {
        if (std::regex_search(tensor->name(), std::regex("nms_postprocess"))) {
            auto detections = nms_decode(tensor, params->labels,
                params->detection_threshold, params->max_boxes, params->filter_by_score);
            hailo_common::add_detections(roi, detections);
        }
    }
}
