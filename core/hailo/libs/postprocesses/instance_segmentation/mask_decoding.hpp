#ifndef _POSTPROCESSES_INSTANCE_SEGMENTATION_MASK_DECODING_HPP_
#define _POSTPROCESSES_INSTANCE_SEGMENTATION_MASK_DECODING_HPP_
...
{
    uint32_t axis_length = matrix_1.shape(2);
    ...
    for (uint32_t i = 0; i < matrix_1.shape(0); ++i) {
        for (uint32_t j = 0; j < matrix_1.shape(1); ++j) {
            row_sum = 0.0;
            for (uint32_t k = 0; k < axis_length; ++k) {
...
void decode_masks(std::vector<HailoDetection> &objects, const xt::xarray<float> &proto)
{
    ...
    for (auto &instance : objects) {
        ...
        if (nullptr == matrix) { // no mask attached
            return;
        }
        ...
        cropped_mask = common::xtensor_sigmoid(cropped_mask);
...
#endif  // _POSTPROCESSES_INSTANCE_SEGMENTATION_MASK_DECODING_HPP_
