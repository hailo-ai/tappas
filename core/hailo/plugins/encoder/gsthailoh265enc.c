...
#define GST_TYPE_HAILOH265ENC_COMPRESSOR (gst_hailoh265enc_compressor_get_type())
static GType
gst_hailoh265enc_compressor_get_type(void)
{
    ...
    static GType hailoh265enc_compressor_type = 0;
    static const GEnumValue hailoh265enc_compressors[] = {
    ...
    if (!hailoh265enc_compressor_type)
    {
        hailoh265enc_compressor_type =
            g_enum_register_static("GstHailoH265EncCompressor", hailoh265enc_compressors);
    }
    return hailoh265enc_compressor_type;
}

#define GST_TYPE_HAILOH265ENC_BLOCK_RC_SIZE (gst_hailoh265enc_block_rc_size_get_type())
static GType
gst_hailoh265enc_block_rc_size_get_type(void)
{
    static GType hailoh265enc_block_rc_size_type = 0;
    static const GEnumValue hailoh265enc_block_rc_size_types[] = {
        {0, "64X64", "64x64"},
        {1, "32X32", "32x32"},
        {2, "16X16", "16x16"},
        {0, NULL, NULL},
    };
    if (!hailoh265enc_block_rc_size_type)
    {
        hailoh265enc_block_rc_size_type =
            g_enum_register_static("GstHailoH265EncBlockRcSize", hailoh265enc_block_rc_size_types);
    }
    return hailoh265enc_block_rc_size_type;
}
...
static void gst_hailoh265enc_init(GstHailoH265Enc * hailoenc);
...
static gboolean gst_hailoh265enc_set_format(GstVideoEncoder * encoder, GstVideoCodecState * state);
static gboolean gst_hailoh265enc_propose_allocation(GstVideoEncoder * encoder, GstQuery * query);
static gboolean gst_hailoh265enc_flush(GstVideoEncoder * encoder);
static gboolean gst_hailoh265enc_start(GstVideoEncoder * encoder);
static gboolean gst_hailoh265enc_stop(GstVideoEncoder * encoder);
static GstFlowReturn gst_hailoh265enc_finish(GstVideoEncoder * encoder);
static GstFlowReturn gst_hailoh265enc_handle_frame(GstVideoEncoder * encoder, GstVideoCodecFrame * frame);
...
G_DEFINE_TYPE_WITH_CODE(GstHailoH265Enc, gst_hailoh265enc, GST_TYPE_VIDEO_ENCODER, _do_init);
...
gst_hailoh265enc_class_init(GstHailoH265EncClass * klass)
{
  ...
  g_object_class_install_property(gobject_class, PROP_COMPRESSOR,
                                    g_param_spec_enum("compressor", "Compressor", "Enable/Disable Embedded Compression",
                                                      GST_TYPE_HAILOH265ENC_COMPRESSOR, (guint)3,
                                                      (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property(gobject_class, PROP_BLOCK_RC_SIZE,
                                    g_param_spec_enum("block-rc-size", "Block Rate Control Size", "Size of block rate control",
                                                      GST_TYPE_HAILOH265ENC_BLOCK_RC_SIZE, (guint)0,
                                                      (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_PLAYING)));
  ...
  venc_class->start = gst_hailoh265enc_start;
  venc_class->stop = gst_hailoh265enc_stop;
  venc_class->finish = gst_hailoh265enc_finish;
  venc_class->handle_frame = gst_hailoh265enc_handle_frame;
  venc_class->set_format = gst_hailoh265enc_set_format;
  venc_class->propose_allocation = gst_hailoh265enc_propose_allocation;
  venc_class->flush = gst_hailoh265enc_flush;
...
static void
gst_hailoh265enc_init(GstHailoH265Enc * hailoenc)
{
  //GstHailoH265EncClass *klass =
  //    (GstHailoH265EncClass *) G_OBJECT_GET_CLASS (hailoenc);
...
gst_hailoh265enc_get_property(GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) (object);
...
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) object;
  GST_DEBUG_OBJECT(hailoenc, "hailoh265enc finalize callback");
 ...
 */
int64_t gst_hailoh265enc_difftimespec_ms(const struct timespec after, const struct timespec before)
{
 ...
 *
 * @param[in] hailoenc     The GstHailoH265Enc object.
 ...
 */
static gboolean 
gst_hailoh265enc_update_params(GstHailoH265Enc *hailoenc, GstVideoInfo * info)
{
 ...
 */
static GstFlowReturn gst_hailoh265enc_update_input_buffer(GstHailoH265Enc * hailoenc,
 ...
 */
static GstBuffer *gst_hailoh265enc_get_encoded_buffer(GstHailoH265Enc * hailoenc)
{
  GstBuffer *outbuf;
  EncoderParams *enc_params = &(hailoenc->enc_params);
 ...
 */
static void
gst_hailoh265enc_set_headers(GstVideoEncoder * encoder)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  GList *headers = NULL;
 ...
 */
static VCEncRet
gst_hailoh265enc_encode_header(GstVideoEncoder * encoder)
{
  VCEncRet enc_ret;
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  EncoderParams *enc_params = &(hailoenc->enc_params);
  ...
  enc_ret = VCEncStrmStart(hailoenc->hevc_encoder, pEncIn, pEncOut);
  if (enc_ret != VCENC_OK)
  {
    return enc_ret;
  }
  hailoenc->header_buffer = gst_hailoh265enc_get_encoded_buffer(hailoenc);
  gst_hailoh265enc_set_headers(encoder);
...
gst_hailoh265enc_stream_restart(GstVideoEncoder * encoder, GstVideoCodecFrame * frame)
{
  VCEncRet enc_ret;
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  EncoderParams *enc_params = &(hailoenc->enc_params);
  ...
  {
    frame->output_buffer = gst_hailoh265enc_get_encoded_buffer(hailoenc);
  ...
  {
    if (UpdateEncoderConfig(&(hailoenc->hevc_encoder), enc_params) != 0)
    {
  ...
  enc_ret = gst_hailoh265enc_encode_header(encoder);
  if (enc_ret != VCENC_OK)
  {
 ...
 */
static GstFlowReturn
gst_hailoh265enc_send_slice(GstHailoH265Enc * hailoenc, u32 *address, u32 size)
{
  GstBuffer *outbuf;
  GstVideoCodecFrame *frame;
 ...
 * @return void
 */
static void gst_hailoh265enc_slice_ready(VCEncSliceReady *slice)
{
  u32 i;
  u32 streamSize;
  u8 *strmPtr;
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) slice->pAppData;
  EncoderParams *enc_params = &(hailoenc->enc_params);
  ...
  if(enc_params->multislice_encoding)
  {
    if (slice->slicesReadyPrev == 0)    /* New frame */
    {
        ...
        gst_hailoh265enc_send_slice(hailoenc, (u32 *)strmPtr, streamSize);
    }
    else
    {
 ...
 * @return Upon success, returns GST_FLOW_OK, GST_FLOW_ERROR on failure.
 */
static GstFlowReturn encode_single_frame(GstHailoH265Enc *hailoenc, GstVideoCodecFrame * frame)
{
  ...
  ret = gst_hailoh265enc_update_input_buffer(hailoenc, frame);
  if (ret != GST_FLOW_OK)
  {
  ...
  enc_ret = EncodeFrame(enc_params, hailoenc->hevc_encoder, &gst_hailoh265enc_slice_ready, hailoenc);
  clock_gettime(CLOCK_MONOTONIC,&end_encode);
  GST_DEBUG_OBJECT(hailoenc, "Encode took %lu milliseconds", (long)gst_hailoh265enc_difftimespec_ms(end_encode,start_encode));
  ...
  switch (enc_ret)
  {
    case VCENC_FRAME_READY:
      ...
      else
      {
        ...
        UpdateEncoderGOP(enc_params, hailoenc->hevc_encoder);
      }
      break;
    default:
 ...
 */
static GstFlowReturn
gst_hailoh265enc_encode_frames(GstVideoEncoder * encoder)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  EncoderParams *enc_params = &(hailoenc->enc_params);
...
static gboolean
gst_hailoh265enc_set_format(GstVideoEncoder * encoder, GstVideoCodecState * state)
{
  // GstCaps *other_caps;
  ...
  gboolean updated_caps = gst_hailoh265enc_update_params(hailoenc, &(state->info));
  if (hailoenc->hevc_encoder != NULL && updated_caps)
  {
    GST_INFO_OBJECT(hailoenc, "Encoder parameters changed, restarting encoder");
    hailoenc->stream_restart = TRUE;
    hailoenc->hard_restart = TRUE;
  }
  else if (hailoenc->hevc_encoder == NULL)
  {
    ...
    VCEncRet ret = gst_hailoh265enc_encode_header(encoder);
    if (ret != VCENC_OK)
    {
...
static gboolean
gst_hailoh265enc_propose_allocation(GstVideoEncoder * encoder, GstQuery * query)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  GST_DEBUG_OBJECT(hailoenc, "hailoh265enc propose allocation callback");
...
static gboolean
gst_hailoh265enc_flush(GstVideoEncoder * encoder)
{
  return TRUE;
}

static gboolean
gst_hailoh265enc_start(GstVideoEncoder * encoder)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  EncoderParams *enc_params = &(hailoenc->enc_params);
  VCEncIn * pEncIn = &(enc_params->encIn);
...
static gboolean
gst_hailoh265enc_stop(GstVideoEncoder * encoder)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  EncoderParams *enc_params = &(hailoenc->enc_params);
  CloseEncoder(hailoenc->hevc_encoder);
...
static GstFlowReturn
gst_hailoh265enc_finish(GstVideoEncoder * encoder)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  ...
  return gst_pad_push(encoder->srcpad, gst_hailoh265enc_get_encoded_buffer(hailoenc));
}

static GstFlowReturn
gst_hailoh265enc_handle_frame(GstVideoEncoder * encoder,
    GstVideoCodecFrame * frame)
{
  GstHailoH265Enc *hailoenc = (GstHailoH265Enc *) encoder;
  GstFlowReturn ret = GST_FLOW_ERROR;
  ...
  {
    ret = gst_hailoh265enc_stream_restart(encoder, frame);
    if (ret != GST_FLOW_OK)
    {
  ...
  if (GST_VIDEO_CODEC_FRAME_IS_FORCE_KEYFRAME(frame))
  {
    ...
    ForceKeyframe(enc_params, hailoenc->hevc_encoder);
  ...
  switch (enc_params->nextCodingType)
  {
    ...
    case VCENC_PREDICTED_FRAME:
      ...
      {
        ret = gst_hailoh265enc_encode_frames(encoder);
      }
  ...
  GST_DEBUG_OBJECT(hailoenc, "handle_frame took %lu milliseconds", (long)gst_hailoh265enc_difftimespec_ms(end_handle,start_handle));
  return ret;
}
